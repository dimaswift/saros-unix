#!/usr/bin/env python3
"""
Parse NASA Solar Saros eclipse series pages into JSONL + JSON output.

Usage:
    python parse_solar_saros.py <saros_number>
...
OUTPUT_BASE = os.path.join(SCRIPT_DIR, "solar")
...
ECLIPSE_RE = re.compile(
    ...
    r"(-?\d+)\s+"               # luna_num
    r"(\w+[+\-]?)\s+"           # ecl_type (may include +/- suffix e.g. A+, A-, T-)
    r"(-?[\d.]+)\s+"            # gamma
    r"([\d.]+)\s+"              # magnitude
